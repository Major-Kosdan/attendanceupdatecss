...
class attendanceDetails
{
    ...
    public function getAttenDanceReport($dbo,$session,$course,$fac)
    {
        ...
        try{
            ...
            $sessionName=$sd['year']." ".$sd['semester'];
        }
        catch(Exception $e)
        {
        ...
        $c="select * from staff_details where id=:id";
        $s=$dbo->conn->prepare($c);
        try{
            $s->execute([":id"=>$fac]);
            $sd=$s->fetchAll(PDO::FETCH_ASSOC)[0];
        ...
        array_push($report,["Staff:",$facname]);
        //first get the total number of classes by fac
        $total=0;
        $start='';
        $end='';
        ...
        $s=$dbo->conn->prepare($c);
        try{
            $s->execute([":session_id"=>$session,":course_id"=>$course,":staff_id"=>$fac]);
            $rv=$s->fetchAll(PDO::FETCH_ASSOC);
            $total=count($rv);
            if($total>0)
            {
        ...
        */
        $rv=[];
        $c="select rsd.id,rsd.reg_no,rsd.name,count(ad.on_date) as attended from
            (
                select sd.id,sd.reg_no,sd.name,crd.session_id, 
                crd.course_id from student_details as sd,course_registration
                as crd
        ...
        array_push($report,["S/N","Reg_No","Name","Total_Classes","Percentage"]);
